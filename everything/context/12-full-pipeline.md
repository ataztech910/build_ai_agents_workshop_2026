# CONTEXT: Полный код pipeline.ts

```typescript
/**
 * Lead Finder Pipeline v3
 * Главный оркестратор — запускает парсер и двух агентов последовательно
 *
 * Запуск:
 *   cp .env.example .env  → настрой SOURCE= и провайдер
 *   npm start
 */
...
async function main() {
  ...
  // Шаг 1: парсим через gramjs / YouTube API / snoowrap / Playwright
  ...
  // Шаг 3: пишем офферы через Claude
  ...
  writeFileSync(
    file,
    JSON.stringify({ source, parsed, leads: leadsData, offers: offersData }, null, 2)
  );
  console.log(`\n💾 Сохранено: ${file}`);
}
